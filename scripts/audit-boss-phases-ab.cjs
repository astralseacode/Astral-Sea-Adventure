// Paired, seeded boss phase A/B audit. Uses the real Worker in an isolated VM and in-memory KV.
// Usage: node scripts/audit-boss-phases-ab.cjs [region-id]
...
async function main(){
  ...
  const result={method:{sourceHash,seedsPerConfig,actionCap,regions:selected,
    ...
    actionCapMeaning:'80 attempted commands, separate cap outcome'},
    ...
    byRegionLevel:pairSummary(all,['region','level','levelBand']),
    ...
    byConfiguration:pairSummary(all,['region','level','bossId','className','strategy']),
    caps:all.filter(x=>x.outcome==='cap'),rows:all};
  const suffix=selected.length===1?`-${selected[0]}`:'';
  const output=path.join(root,`boss-phase-ab-results${suffix}.json`);
