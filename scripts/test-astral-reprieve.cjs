...
const activation = 'No Stims Used - Astral Reprieve +30 Mana';
const unlock = 'lvl 38 Passive ✨ Astral Reprieve: Defeating an enemy without using Stim during the battle restores 30 Mana.';
...
(async () => {
  ...
  assert.doesNotMatch(low.result.message, /Astral Reprieve/);
  ...
  assert.match(normal.result.message, /Astral Harvest activates!/);
  assert.equal(normal.progress.mana, 50);
  assert.equal(await normal.f.state(), null);
  const high = await victory(39);
  assert.match(high.result.message, /No Stims Used - Astral Reprieve \+30 Mana/);
  ...
  assert.match(spellWin.message, /No Stims Used - Astral Reprieve \+30 Mana/);
  assert.equal(await spell.state(), null);

  const stacked = await victory(38, 0, 20);
  assert.match(stacked.result.message, /Astral Defiance activates!/);
  ...
  assert.match(capped.result.message, /No Stims Used - Astral Reprieve \+30 Mana/);
  const full = await victory(38, 100);
  assert.equal(full.progress.mana, 100);
  assert.match(full.result.message, /No Stims Used - Astral Reprieve \+30 Mana/);
  ...
  assert.match((await rejected.attack()).message, /No Stims Used - Astral Reprieve \+30 Mana/);
  ...
  used.rolls.push(2);
  assert.doesNotMatch((await used.attack()).message, /Astral Reprieve/);
  await used.editProgress(p => { p.mana = 0; });
  await used.c.startCombatEncounter(used.env, used.key,
    used.c.getRegionById('moonlit-reef'), 1, used.enemy, 'discord');
  await used.editState(s => { s.enemy.hp = 1; });
  used.rolls.push(2);
  assert.match((await used.attack()).message, /No Stims Used - Astral Reprieve \+30 Mana/);
  ...
  assert.match((await buffered.attack()).message, /No Stims Used - Astral Reprieve \+30 Mana/);
  assert.equal((await buffered.progress()).mana, 175);

  const twitch = await victory(38, 0, 100, 'twitch');
  assert.match(twitch.result.message, /No Stims Used - Astral Reprieve \+30 Mana/);
  ...
  assert.doesNotMatch(notice.message, /No Stims Used - Astral Reprieve/);
  assert.equal((await levelUp.c.formatLevelUpUnlocks(37, 38)).join('\n'), unlock);
  console.log('Astral Reprieve regressions passed.');
