...
(async () => {
  ...
  assert(spell);
  assert.equal(spell.choices.length, 16);
  assert.equal(spell.choices.filter(choice => choice.name === 'Help!').length, 1);
  assert.equal(spell.choices.find(choice => choice.name === 'Help!').value, 'help');
