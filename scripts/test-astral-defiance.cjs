...
(async () => {
  ...
  assert.doesNotMatch(low.result.message, /Defiance activates!/);
  assert.equal(low.progress.hp, 52);
  assert.equal(low.progress.mana, 20);

  const below = await lethal(36, 37);
  assert.match(below.result.message, /Harvest activates!/);
  assert.match(below.result.message, /Defiance activates!\n\n\+20 HP \+20 Mana/);
  ...
  assert.doesNotMatch(above.result.message, /Defiance activates!/);
  ...
  capped.rolls.push(2);
  assert.match((await capped.attack()).message, /Defiance activates!/);
  ...
  assert.doesNotMatch((await nonlethal.attack()).message, /Defiance activates!/);
  assert(await nonlethal.state());

  const exact = await lethal(37, 43, 'short');
  assert.match(exact.result.message, /Defiance activates!/);
  assert.equal(exact.progress.hp, 78);
  const tooHigh = await lethal(37, 44, 'short');
  assert.doesNotMatch(tooHigh.result.message, /Defiance activates!/);

  const twitch = await lethal(36, 37, null, 'twitch');
  assert.match(twitch.result.message, /Defiance activates!\n\n\+20 HP \+20 Mana/);
  console.log('Defiance focused regressions passed.');
